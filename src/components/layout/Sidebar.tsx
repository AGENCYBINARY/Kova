    ...
    async function loadIntegrations() {
      try {
        ...
        setIntegrations([
          {
            name: 'Gmail',
            status: data.items.find((item: { id: string; status: string }) => item.id === 'gmail')?.status || 'disconnected',
            color: '#EA4335',
          },
          {
            name: 'Calendar',
            status: data.items.find((item: { id: string; status: string }) => item.id === 'calendar')?.status || 'disconnected',
            color: '#4285F4',
          },
          {
            name: 'Notion',
            status: data.items.find((item: { id: string; status: string }) => item.id === 'notion')?.status || 'disconnected',
            color: '#000000',
          },
          {
            name: 'Docs',
          ...
          {
            name: 'Slack',
            status: data.items.find((item: { id: string; status: string }) => item.id === 'slack')?.status || 'disconnected',
            color: '#4A154B',
          },
        ])
      } catch {
        // Keep the default disconnected state when the API is unavailable.
      }
    }
